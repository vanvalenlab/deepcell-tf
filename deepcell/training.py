...
def train_model_watershed(model=None, dataset=None, optimizer=None,
    ...
    # fit the model on the batches generated by datagen.flow()
    loss_history = model.fit_generator(
        datagen.flow(train_dict, batch_size=batch_size, distance_bins=distance_bins, skip=skip),
        steps_per_epoch=train_dict['y'].shape[0] // batch_size,
        epochs=n_epoch,
        class_weight=class_weight,
        validation_data=datagen_val.flow(test_dict, batch_size=batch_size, distance_bins=distance_bins, skip=skip),

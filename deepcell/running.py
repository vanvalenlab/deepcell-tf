...
def run_model(image, model, win_x=30, win_y=30, split=True):
    ...
    if split:
        ...
        if CHANNELS_FIRST:
            ...
            model_output[:, 0:image_size_x-win_x, 0:image_size_y-win_y] = model.predict([img_0, 0])[0]
            model_output[:, 0:image_size_x-win_x, image_size_y-win_y:] = model.predict([img_1, 0])[0]
            model_output[:, image_size_x-win_x:, 0:image_size_y-win_y] = model.predict([img_2, 0])[0]
            model_output[:, image_size_x-win_x:, image_size_y-win_y:] = model.predict([img_3, 0])[0]
        else:
            img_0 = image[:, 0:image_size_x+win_x, 0:image_size_y+win_y, :]
            img_1 = image[:, 0:image_size_x+win_x, image_size_y-win_y:, :]
            img_2 = image[:, image_size_x-win_x:, 0:image_size_y+win_y, :]
        ...
        model_output = model.predict([image, 0])[0]

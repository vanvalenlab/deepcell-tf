...
def identity_block(X, f, filters, stage, block):
    ...
    X = Conv3D(filters=F2, kernel_size=(1, f, f), strides=(1, 1, 1), padding='same', name=conv_name_base + '2b',
               kernel_initializer=glorot_uniform(seed=0), data_format = 'channels_first')(X)
...
def convolutional_block(X, f, filters, stage, block, s=2):
    ...
    ##### MAIN PATH #####
    # First component of main path
    X = Conv3D(F1, (1, 1, 1), strides=(1, s, s), name=conv_name_base + '2a', kernel_initializer=glorot_uniform(seed=0), data_format = 'channels_first')(X)
    X = BatchNormalization(axis=1, name=bn_name_base + '2a')(X)
    X = Activation('relu')(X)
    ...
    ##### SHORTCUT PATH ####
    X_shortcut = Conv3D(filters=F3, kernel_size=(1, 1, 1), strides=(1, s, s), padding='valid', name=conv_name_base + '1',
...
def ResNet50(input_shape, classes=6):
    ...
    # Stage 1
    X = Conv3D(64, (7, 7, 7), strides=(1, 2, 2), name='conv1', kernel_initializer=glorot_uniform(seed=0), data_format = 'channels_first')(X)
    ...
    # AVGPOOL
    X = AveragePooling3D((3, 7, 7), name="avg_pool")(X)
...
def main():
    ...
    X = X[np.newaxis]
    print('Xnewaxis.shape: {}\ny.shape: {}'.format(X.shape, y_train.shape))

    model = ResNet50(input_shape=X_train_2.shape[1:], classes=6)
    print(model.summary())

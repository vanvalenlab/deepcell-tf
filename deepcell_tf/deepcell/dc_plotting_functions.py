...
def plot_training_data_2d(X, y, max_plotted=5):
    ...
    channels_index = 1 if image_data_format == 'channels_first' else -1

    if max_plotted > y.shape[0]:
        max_plotted = y.shape[0]

    fig, ax = plt.subplots(max_plotted, y.shape[channels_index] + 1, squeeze=False)

    for i in range(max_plotted):
        X_i = X[i, 0, :, :] if image_data_format == 'channels_first' else X[i, :, :, 0]
        ...
        for j in range(1, y.shape[channels_index] + 1):
            y_k = y[i, j - 1, :, :] if image_data_format == 'channels_first' else y[i, :, :, j - 1]
...
def plot_training_data_3d(X, y, num_image_stacks, frames_to_display=5):
    ...
    for i in range(num_image_stacks):
        ...
        for j in range(frames_to_display):
            y_j = y[i, j, :, :] if image_data_format == 'channels_first' else y[i, :, :, j]
